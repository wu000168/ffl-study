...
function generateTasks(taskIds: { formula: number, tool: "FFL" | "LaTeX" | "Word" }[], participantID: string) {
  return taskIds.map(({ formula, tool }, idx) => {
    let task = tasks[formula - 1];
    if (task) {
      switch (tool) {
        case "FFL": return (<MdEditor
...
function App() {
  ...
  const getReport = () =>
    "Participant ID,Task 1, Task 2, Task 3, Task 4,Total Time\n" +
    `${participantID},${taskTimes.current.map(s => s.toString()).join(',')},${studyEnd.current - studyStart.current}`
  return (
    <div className="App">
      <Container maxWidth='lg' sx={{ height: '100vh', padding: '8pt' }}>
        <SteppedComponent activeStep={started}>
          ...
          <Box sx={{ height: '100%', width: '100%', maxHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            <Stepper sx={{ flex: '0', width: '100%' }} activeStep={activeStep}>
              ...
              <Step>
                <StepLabel optional={<Typography variant="caption">Tool A</Typography>}>Task D</StepLabel>
              </Step>
              <Step>
                <StepLabel>Done</StepLabel>
              </Step>
            </Stepper>
            <Box sx={{ flex: '1 1', pt: 2 }}>
