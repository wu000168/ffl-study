...
function FFLTask({ md, ffl, ratio, task, target, onChange }: any) {
    ratio ??= [4, 3];
        ...
        <Stack spacing={2} sx={{ height: '100%', width: '100%' }} direction='column'>
            ...
            >
                <Stack spacing={2} sx={{ flex: 1, height: "100%", display: 'flex', flexDirection: "column" }}>
                    ...
                    />
                    <div style={{ textAlign: 'start', marginTop: '8pt', marginBottom: '-8pt' }}>Style Specification {errMsg ? <Chip size="small" variant="outlined" color="error" label="Invalid/Incomplete Syntax" /> : ""}</div>
                    <textarea spellCheck={false}
                        style={{
